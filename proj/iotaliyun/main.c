...
#define DEVICE_NAME         "onlinetest05"
#define DEVICE_SECRET       "vCJGHUnZAP21gmQk6xwGumm0a2cxiWrM"
#define KEEPALIVE	180
//lzw 定义的产品设备的唯一id
#define EQUIPMENTID         "2c91e1c95fb42ec0015fb4522a570005"
...
void send_message_to_view()
{
    ...
    for (i = 0; i < cnt; i++)
    {
        if(i==0)
        {
             ...
             sprintf(init_state_iot,"%s",dst[i]);
...
#endif
	{
        //收到服务器消息，进行截取解析 1 dst 业务变量放入本地缓存 2 cnt 获得变量个数（长度）
        cnt = split(dst, message->payload, "#");

		//测试用----------------------------------
		//sprintf(tbuf,"-------------%02d:%02d:%02d-----------",cnt);
		//----------------------------------------
...
void mlink_callback_t(sc_status status, void *pdata)
{
	...
	switch(status) {
		case SC_STATUS_WAIT:
		    uart0_sendStr("page a_g0");
            uart0_tx_buffer(ascii,3);
            //先切换屏幕界面给个延时
            sys_msleep(300);

            uart0_sendStr("a_msg.txt=\"配置网络模式  请使用手机APP进行设置\"");
            uart0_tx_buffer(ascii,3);
...
//		uart0_sendStr("SC_STATUS_GETTING_SSID_PSWD\r\n");
		break;
		case SC_STATUS_LINK:
		uart0_sendStr("a_msg.txt=\"收到上网口令！！ 正在连接网络（如果长时间未能连接请重置并检查口令是否正确）\"");
        uart0_tx_buffer(ascii,3);
...
//        sprintf(tbuf,"t7.txt=\"%d-%d-%d %d:%d:%d\"",t->tm_year + 1900,t->tm_mon + 1,t->tm_mday,t->tm_hour,t->tm_min,t->tm_sec);
		...
		//sprintf(tbuf,"times.txt=\"%s\"",tmp);
        sprintf(tbuf,"times.txt=\"%02d:%02d:%02d\"",t->tm_hour,t->tm_min,t->tm_sec);
		uart0_sendStr(tbuf);
		uart0_tx_buffer(ascii,3);
		sys_msleep(1000);

		if(t->tm_hour == 1 && t->tm_min == 0 && t->tm_sec == 0){
			while(sntp_init("time.pool.aliyun.com") != 0)
			{
				sys_msleep(100);
			}
...
//		uart0_sendStr("start mlink\r\n");
        ...
        //先切换屏幕界面给个延时
        sys_msleep(300);
	    uart0_sendStr("a_msg.txt=\"正在连接网络 请稍后\"");
        uart0_tx_buffer(ascii,3);
	}
#else
	//读取当前模块的STA配置信息
