...
coap_print_addr(const struct coap_address_t *addr, unsigned char *buf, size_t len) {
  ...
  p += sprintf((char *)p, buf + len - p + 1, ":%d", port);
...
#ifdef HAVE_SNPRINTF
  p += sprintf((char *)p, buf + len - p + 1, ":%d", uip_htons(addr->port));
#else /* HAVE_SNPRINTF */
...
msg_code_string(uint8_t c) {
  ...
  if (c < sizeof(methods)/sizeof(char *)) {
    ...
    sprintf(buf, /*sizeof(buf),*/ "%u.%02u", c >> 5, c & 0x1f);
    return buf;
  }
}
...
msg_option_string(uint16_t option_type) {
  ...
  sprintf(buf, sizeof(buf), "%u", option_type);
  return buf;
}
...
print_content_format(unsigned int format_type,
  ...
  for (i = 0; i < sizeof(formats)/sizeof(struct desc_t); i++) {
    if (format_type == formats[i].type) {
      return sprintf((char *)result, buflen, "%s", formats[i].name);
    }
  }

  /* unknown content format, just print numeric value to buf */
  return sprintf((char *)result, buflen, "%d", format_type);
}
...
coap_show_pdu(const coap_pdu_t *pdu) {
  ...
  while ((option = coap_option_next(&opt_iter))) {
    ...
    case COAP_OPTION_BLOCK2:
      ...
      buf_len = sprintf((char *)buf, sizeof(buf), "%u/%c/%u",
    ...
    case COAP_OPTION_SIZE1:
      ...
      buf_len = sprintf((char *)buf, sizeof(buf), "%u",
