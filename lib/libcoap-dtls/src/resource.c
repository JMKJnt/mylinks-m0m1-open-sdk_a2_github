...
#include "subscribe.h"
#include "utlist.h"
#if defined(WITH_LWIP) && !defined(MONT_LWIP)
...
#if defined(WITH_POSIX)||defined(MONT_LWIP)
...
static void* _memchr(unsigned char* s, unsigned char c, size_t n) {
	unsigned char *p = s;
...
match(const str *text, const str *pattern, int match_prefix, int match_substring) {
  ...
  if (match_substring) {
    ...
    while (remaining_length) {
      ...
      next_token = _memchr(token, ' ', remaining_length);
