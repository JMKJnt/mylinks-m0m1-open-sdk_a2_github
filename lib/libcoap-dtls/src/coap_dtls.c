...
flush_data(struct coap_dtls_context_t *context,
  ...
  LL_FOREACH_SAFE(session->sendqueue, item, tmp) {
    ...
    if (res <= 0) {
      ...
      serial_printf("data not written\n");
      return ok;
    } else {
      if ((size_t)res < item->data_length) {
        //debug("data truncated by dtls_write()\n");
        serial_printf("data truncated by dtls_write()\n");
      } else {
        ok = 1;
      }
...
dtls_send_to_peer(struct dtls_context_t *dtls_context,
  ...
  result = udp_sendto(coap_context->endpoint->pcb, p,
...
dtls_event(struct dtls_context_t *dtls_context,
  ...
  LL_FOREACH(coap_context->dtls_context->sessions, session) {
    if ((session->dtls_session.ifidx == dtls_session->ifidx) &&
...
static dtls_handler_t cb = {
  .write_cb = dtls_send_to_peer,
  .read_cb  = dtls_application_data,
  .event = dtls_event,
...
#define CTX_SIZE (sizeof(struct coap_dtls_context_t))

  context = (struct coap_dtls_context_t *)coap_malloc(CTX_SIZE);
  if (context) {
...
coap_dtls_new_session(coap_dtls_context_t *dtls_context,
  ...
  if (session) {
    ...
    session->dtls_session.ifidx = local_interface->ifindex;
...
coap_dtls_find_session(coap_dtls_context_t *dtls_context,
  ...
  LL_FOREACH(dtls_context->sessions, session) {
    if ((session->dtls_session.ifidx == local_interface->ifindex) &&
