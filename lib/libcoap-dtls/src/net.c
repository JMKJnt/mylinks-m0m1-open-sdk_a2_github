...
coap_new_context(
  ...
  if (listen_addr) {
	if(coap_dtls_is_supported())
    	coap_attach_endpoint(c, coap_new_endpoint(listen_addr, COAP_ENDPOINT_DTLS));
	else
...
coap_handle_message(coap_context_t *ctx,
  ...
  node->pdu = (coap_pdu_t *)coap_pdu_from_payload((void*)msg, msg_len);
#else
