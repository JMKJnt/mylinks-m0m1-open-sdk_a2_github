...
#define DEVICE_NAME         "onlinetest03"
#define DEVICE_SECRET       "oTfFQvEQO1I2IMtbrmMhyIvP3vPaRO8L"
#define KEEPALIVE	180
//lzw 定义的产品设备的唯一id
#define EQUIPMENTID         "2c91e1c95fb42ec0015fb450fb9e0003"
...
//        uint8_t ascii[3] ={0xff,0xff,0xff};
        char dst[10][80];
...
//                    puts(dst[i]);
         char initeq[2]="0";
         char init_state_iot[20];
         sprintf(init_state_iot,"%s",dst[8]);
//         uart0_sendStr(dst[8]);

//         scanf("%s",in_str);//输入字符串,这种方式字符串之间不能有空格.如果有空格的话,可以用gets(in_str);
//        if(strcmp(in_str,cmp_str)==0){    //如果相等的话返回值为0
//         printf("yes\n");
//         }
//         else{
//         printf("no\n");
//         }
         ...
         sprintf(a_p0,"p0.pic=%s",dst[0]);
         uart0_sendStr(a_p0);
         uart0_tx_buffer(ascii,3);

         sprintf(a_t0,"t0.txt=\"%s\"",dst[1]);
         uart0_sendStr(a_t0);
         uart0_tx_buffer(ascii,3);

         sprintf(a_t1,"t1.txt=\"%s\"",dst[2]);
         uart0_sendStr(a_t1);
         uart0_tx_buffer(ascii,3);

         sprintf(a_t2,"t2.txt=\"%s\"",dst[3]);
         uart0_sendStr(a_t2);
         uart0_tx_buffer(ascii,3);

         sprintf(a_t3,"t3.txt=\"%s\"",dst[4]);
         uart0_sendStr(a_t3);
         uart0_tx_buffer(ascii,3);

         sprintf(a_t4,"t4.txt=\"%s\"",dst[5]);
         uart0_sendStr(a_t4);
         uart0_tx_buffer(ascii,3);

         sprintf(a_t5,"t5.txt=\"%s\"",dst[6]);
         uart0_sendStr(a_t5);
         uart0_tx_buffer(ascii,3);

         sprintf(a_t6,"t6.txt=\"%s\"",dst[7]);
         uart0_sendStr(a_t6);
         uart0_tx_buffer(ascii,3);
