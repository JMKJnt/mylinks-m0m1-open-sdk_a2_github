...
//                    puts(dst[i]);
         ...
         sprintf(init_state_iot,"%s",dst[5]);
         ...
         sprintf(p0,"p0.pic=%s",dst[0]);
         uart0_sendStr(p0);
         uart0_tx_buffer(ascii,3);

         sprintf(nt,"nt.txt=\"%s\"",dst[1]);
         uart0_sendStr(nt);
         uart0_tx_buffer(ascii,3);

         sprintf(t1,"t1.txt=\"%s\"",dst[2]);
         uart0_sendStr(t1);
         uart0_tx_buffer(ascii,3);

         sprintf(t2,"t2.txt=\"%s\"",dst[3]);
         uart0_sendStr(t2);
         uart0_tx_buffer(ascii,3);

         sprintf(city,"city.txt=\"%s\"",dst[4]);
         uart0_sendStr(city);
         uart0_tx_buffer(ascii,3);
...
//        sprintf(tbuf,"t7.txt=\"%d-%d-%d %d:%d:%d\"",t->tm_year + 1900,t->tm_mon + 1,t->tm_mday,t->tm_hour,t->tm_min,t->tm_sec);
        sprintf(tbuf,"t4.txt=\"%d:%d:%d\"",t->tm_hour,t->tm_min,t->tm_sec);
		uart0_sendStr(tbuf);
		uart0_tx_buffer(ascii,3);
		sys_msleep(1000);
	}
